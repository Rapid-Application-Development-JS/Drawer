(function (root, factory) {
  ...
  function addVendorPrefix(property) {
    var arr = ['ms', 'moz', 'webkit', 'o'], index, tmp = document.createElement('div'),
    ...
    for (index = 0; index < arr.length; index += 1) {
      arrayOfPrefixes.push(arr[index] + capitalise(property));
    }
    for (index = 0; index < arrayOfPrefixes.length; index += 1) {
      if (tmp.style[arrayOfPrefixes[index]] !== undefined) {
        result = '-' + arr[index] + '-' + property;
        break;
      }
    }
    return result;
   ...
   * @param {HTMLElement} wrapper Drawer  should contain two main elements.
   * First element will be used as drawer layer, second element - as main content layer
   * @param {Object=} options Object with options fields. If options does not set Drawer will be initialized with
   * default parameters, if options does not have one or more fields these fields will be initialized with default values
   * @constructor
   * @version 1.0.0
   */
  function Drawer(wrapper, options) {
    ...
    function createBehavior() {
      var behavior = {},
        ...
        getGestureKindRight = function (touchPoint) {
          if (touchPoint.x < behavior.wrapperOffset - behavior.trackingSize && _isClosed === true) {
            return orientationType.vertical; // don't track it
          }
          return null;
        },
        getGestureKindTop = function (touchPoint) {
          if (touchPoint.y > behavior.trackingSize && _isClosed === true) {
            return orientationType.horizontal; // don't track it
          }
          return null;
        },
      ...
      behavior.isHorizontalOrientation = _options.align === 'left' || _options.align === 'right';
    ...
    function _pointerMove(event) {
      ...
      if (_behavior.isHorizontalOrientation) {
        _currentShiftForRaf += (X - _lastPoint.x);
        ...
        _currentShiftForRaf += (Y - _lastPoint.y);
    ...
    function _pointerFling(event) {
      ...
      var horizontalFling = Math.abs(event.speedX) > Math.abs(event.speedY);
      var verticalFling = Math.abs(event.speedY) > Math.abs(event.speedX);
      var direction = _behavior.getDirection(event);
      if (
        (
          (horizontalFling && Math.abs(event.speedX) > 0.2 && _enabled) &&
          (_gestureKind === orientationType.horizontal) &&
          _behavior.isHorizontalOrientation) || (
          (verticalFling && Math.abs(event.speedY) > 0.2 && _enabled) &&
          (_gestureKind === orientationType.vertical) &&
          !_behavior.isHorizontalOrientation
        )
      ) {
        clearTimeout(_tweatID);
    ...
    function _action(activity) {
      var shift, animTime = _options.animationTime;
      ...
      _content.style[scope._transitionName] =
        'all ' + (animTime * Math.abs(shift) / _wrapperWidth) + 'ms ease-in-out';
      if (_options.overlay) {
        _overlay.style.opacity = Math.abs(_contentPosition / _wrapperWidth);
        _overlay.offsetWidth; //dog bone
        _overlay.style[scope._transitionName] =
          'opacity ' + (animTime * Math.abs(shift) / _wrapperWidth) + 'ms ease-in-out';
